...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.commands.registerCommand('epub.saveFile', async () => {		try {
	...
	context.subscriptions.push(vscode.commands.registerCommand('epub.saveFileWithConfirmation', async () => {		try {
			const modifiedCount = epubFs.getModifiedFilesCount();
			if (modifiedCount === 0) {
				vscode.window.setStatusBarMessage('No changes to save.', 3000);
				return;
			...
			const result = await vscode.window.showWarningMessage(
				`Save EPUB with ${modifiedCount} modified file${modifiedCount > 1 ? 's' : ''}?`,
				{ modal: true },
				'Save',
				'Cancel'
			);
	...
	context.subscriptions.push(vscode.commands.registerCommand('epub.listModifiedFiles', async () => {		try {
			...
			const fileList = modifiedFiles.map((file: string) => `• ${file}`).join('\n');
	...
	context.subscriptions.push(vscode.commands.registerCommand('epub.refresh', async () => {		try {
			await vscode.commands.executeCommand('workbench.files.action.refreshFilesExplorer');
...
async function collectEpubMetadata(): Promise<EpubMetadata | undefined> {
	try {
		// Title (required)
