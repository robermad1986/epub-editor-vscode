...
export class EpubFileSystemProvider implements vscode.FileSystemProvider {
	...
	private epubZip?: JSZip;
	private originalEpubPath?: string;
	private isModified = false;
	private currentEpubName?: string; // Track the current EPUB name
	private modifiedFiles = new Set<string>(); // Track which files have been modified
	private pendingSave = false; // Track if a save operation is in progress
	...
	private updateStatusBar(): void {
		...
		if (modifiedCount > 0) {
			vscode.window.setStatusBarMessage(
				`📚 EPUB: ${modifiedCount} file${modifiedCount > 1 ? 's' : ''} modified`, 
				3000
			);
		}
	}
	...
	private async buildFileSystemFromZip(): Promise<void> {
		if (!this.epubZip) {
	...
	private createFileFromPath(filePath: string, content: Uint8Array): void {
		...
		for (let i = 0; i < pathParts.length - 1; i++) {
			...
			current = current.entries.get(dirName) as Directory;
		}
	...
	private createDirectoryFromPath(dirPath: string): void {
	...
	private async updateZipFromFileSystem(): Promise<void> {
		if (!this.epubZip) {
			return;
		}
	...
	private addDirectoryToZip(directory: Directory, basePath: string): void {
		if (!this.epubZip) {
			return;
		}
	...
	async validateEpubStructure(): Promise<ValidationResult> {
		const errors: string[] = [];
	...
	async getEpubMetadata(): Promise<Partial<EpubMetadata>> {
	...
	private _extractXmlValue(xml: string, tagName: string): string | undefined {
	...
	private _getFileByPath(filePath: string): File | undefined {
		const pathParts = filePath.split('/').filter(part => part.length > 0);
	...
	private _countFiles(directory: Directory): number {
		let count = 0;
	...
	triggerRefresh(uri: vscode.Uri): void {
	...
	private _lookup(uri: vscode.Uri, silent: boolean): Entry | undefined;	private _lookup(uri: vscode.Uri, silent: boolean): Entry | undefined {
	...
	private _getZipPathFromUri(uri: vscode.Uri): string {
	 ...
	 */
	async createNewEpub(metadata: EpubMetadata, filePath: string): Promise<void> {
		try {
			console.log(`[EPUB] Creating new EPUB at: ${filePath}`);
			
			// Generate unique identifier if not provided
			if (!metadata.identifier) {
				metadata.identifier = `urn:uuid:${uuidv4()}`;
			}
	 ...
	 */
	private addMimetypeFile(zip: JSZip): void {
	 ...
	 */
	private addMetaInfFiles(zip: JSZip): void {
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
	 ...
	 */
	private addOEBPSFiles(zip: JSZip, metadata: EpubMetadata): void {
		// Create content.opf
	 ...
	 */
	private generateContentOpf(metadata: EpubMetadata): string {
...
<package version="3.0" unique-identifier="BookId" xmlns="http://www.idpf.org/2007/opf">
	 ...
	 */
	private generateTocNcx(metadata: EpubMetadata): string {
		return `<?xml version="1.0" encoding="utf-8"?>
...
.title-page .author {
	 ...
	 */
	private generateTitlePage(metadata: EpubMetadata): string {
		return `<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE html>
...
<body>
	 ...
	 */
	private generateChapterTemplate(chapterNumber: number): string {
		return `<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
